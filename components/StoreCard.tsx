...
interface Props {
  ...
  onEdit: (store: StoreCompany) => void;
  onDelete: () => void;
  onAddInvoice: () => void;
  onEditExpenses: () => void;
}
...
const StoreCard: React.FC<Props> = ({ 
  ...
  const profitBeforeTax = store.quarterIncome - store.quarterExpenses - invoiceAmount;
  ...
  if (store.taxType === '一般纳税人') {
    ...
    const inputVat = (invoiceAmount / (1 + INPUT_VAT_RATE)) * INPUT_VAT_RATE;
    ...
    const exTaxInvoiced = invoiceAmount / (1 + INPUT_VAT_RATE);
    ...
    // 3. Income Tax
    taxProfit = store.quarterIncome - store.quarterExpenses - invoiceAmount;
  }
      ...
      <div className="p-6 pb-4 pl-7">
        <div className="flex justify-between items-start mb-4 pr-12">
          <div>
            ...
            <p className="text-sm text-gray-500 flex items-center gap-2 mt-1 whitespace-nowrap">
              ...
              <span className={`text-[10px] px-2 py-0.5 rounded border ${store.taxType === '一般纳税人' ? 'bg-blue-100 text-blue-600 border-blue-200' : 'bg-gray-100 text-gray-600 border-gray-200'} flex-shrink-0 mr-1`}>
                  {store.taxType}
              </span>
              <span className={`text-[10px] px-2 py-0.5 rounded border bg-purple-100 text-purple-600 border-purple-200 flex-shrink-0`}>
                  {store.platform}
              </span>
            </p>
          </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className="font-medium text-blue-600">¥{invoiceAmount.toLocaleString()}</span>
          </div>
